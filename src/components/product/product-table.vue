...
<template>
  <el-table stripe :data="data">
    ...
    <el-table-column label="Категория" prop="slug" width="150">
      <template #default="scope">
        <div>
          {{
            scope.row.category?.translates?.find((tr) => tr.language == 'ru')?.title ||
            scope.row.category?.translates?.at(0)?.title
          }}
        </div>
      </template>
    </el-table-column>
    <el-table-column label="Цена" width="150">
      <template #default="scope">
        <div>{{ scope.row.price.toLocaleString() || 0 }} сум</div>
      </template>
    </el-table-column>
