<template>
  ...
  >
    <el-form
      ...
      label-position="top"
    >
      <el-form-item label="Название бренд" prop="title">
      ...
      <el-form-item label="Фотография бренд">
        <el-upload
          v-model:file-list="data.img"
          class="avatar-uploader"
          list-type="picture-card"
          :action="`${url}/api/brand/upload`"
...
const rules = ref({
  title: [
    {
      required: true,
      message: 'Напишите название бренд'
    }
  ]
})
...
const checkingSlug = async () => {
  ...
  lockToggle.value = await api.checkSlug('brand', data.value.slug)
}
